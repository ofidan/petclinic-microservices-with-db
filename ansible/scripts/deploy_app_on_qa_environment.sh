...
AWS_REGION=$AWS_REGION helm repo add stable-petclinic-oliver s3://petclinic-helm-charts-olivertest/stablemyapp/ || echo "repository name already exists"
AWS_REGION=$AWS_REGION helm repo update
helm package k8s/petclinic_chart
AWS_REGION=$AWS_REGION helm s3 push petclinic_chart-${BUILD_NUMBER}.tgz stable-petclinic-oliver
